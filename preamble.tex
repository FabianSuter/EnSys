...
\usepackage[shortlabels]{enumitem}  % list customization
...
\usepackage[version=4]{mhchem}      % Chemische Schreibweise
...
\usepackage[dvipsnames]{xcolor}     % needed for colors
...
\newcommand*\circled[1]{\tikz[baseline=(char.base)]{
            \node[shape=circle,draw,inner sep=2pt] (char) {#1};}} %For cicles around numbers
...
\usepackage{datetime2}              % custom date format for 
\usepackage{array}                  % make multicol tables
%versioning
...
\newcommand\pro{\item[$\bm{+}$]}                         % Command for Pro
\newcommand\con{\item[$\bm{-}$]}                         % Command for Contra
